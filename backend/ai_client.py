...
def retry_with_backoff(func: Callable[[], str], max_retries: int = 3, base_delay: float = 1.0) -> str:
    ...
    raise RuntimeError(
        "Retry mechanism exhausted without returning a response")
...
def get_ai_response(prompt: str, context: str = "") -> str:
    ...
    except (ValueError, GroqError) as e:
